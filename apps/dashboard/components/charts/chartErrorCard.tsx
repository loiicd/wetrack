...
const CATEGORY_META: Record<
  ErrorCategory,
  { icon: React.ElementType; label: string; hint: string; color: string }
> = {
  network: {
    icon: Wifi,
    label: "API-Fehler",
  ...
  unknown: {
    ...
    hint: "Beim Laden des Charts ist ein Fehler aufgetreten.",
...
const ChartErrorCard = ({ title, message }: ChartErrorCardProps) => {
  ...
  const { icon: Icon, label, hint, color } = CATEGORY_META[category];
  ...
  return (
    <Card className="flex h-full flex-col border-destructive/30 bg-destructive/5 overflow-hidden">
